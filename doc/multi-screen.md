...
```
main.ts
  └─ LightPropagationScreen            (Screen<LightPropagationModel, LightPropagationScreenView>)
       ├─ LightPropagationModel         owns all state
       └─ LightPropagationScreenView    owns all visuals
```
...
Mirror the structure of `src/light-propagation-screen/`:
...
existing `LightPropagationScreen.ts`.
...
const icon = new ScreenIcon(
  new Rectangle(0, 0, 548, 373, { fill: LightPropagationColors.accentColorProperty }),
